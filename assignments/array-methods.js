...
let ticketPriceTotal = runners.reduce(function(accumulator, number) {
  return accumulator + number.donation;
},0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I wantperson.firstName.toUpperCase()) you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1  The tournament director wants a list of Last Names in alphabetic order.
let lastNameAlphabetically = runners.map(person => person.last_name);
console.log(lastNameAlphabetically.sort());


// Problem 2  The tournament director wants to acknowledge anyone over a 250 donation.  Please provide a list.
let Over250 = runners.filter(person => person.donation > 250);
console.log(`here is the list of people who donated over 250`)
console.log(Over250);

// Problem 3  The tournament director wants to sell the email addresses of the runners in the race.  Get him a list.
let emails = [];
runners.forEach(person => emails.push(person.email));
console.log(emails);
